...
foreach ($file in $files) {
    ...
    $oldBytes = $file.Length
        ...
        # --- Per-file state (always defined so logging never breaks under StrictMode) ---
        $origVideo = ""
        $origAudio = ""
        $origDV = 0
        $newVideo = ""
        ...
        # Skip files that may still be copying/downloading (prevents half-written encodes)
        $recentMin = TryParse-Int $Global:MediaConfig.RecentMinutes 0
        if ($recentMin -gt 0) {
            $ageMin = (New-TimeSpan -Start $file.LastWriteTime -End (Get-Date)).TotalMinutes
            if ($ageMin -lt $recentMin) {
                $detail = ("LastWrite {0} ({1:N1} min ago) < {2} min" -f $file.LastWriteTime, $ageMin, $recentMin)
                Write-ShrinkLog -InputPath $file.FullName -Old_MB $oldMB -Status "Skipped-TooNew" -Strategy "too-new" -Detail $detail -OrigVideo $origVideo -OrigAudio $origAudio -OrigDV $origDV -Encode10 0 -AudioPlan "skip-too-new"
                continue
            }
        }
        $encode10 = $false
    ...
    if (-not $v) {
        Write-ShrinkLog -InputPath $file.FullName -Old_MB $oldMB -Status "Skipped-NoVideo" -Detail "No primary video stream" -OrigVideo $origVideo -OrigAudio $origAudio -OrigDV $origDV -Encode10 ([int]$encode10) -AudioPlan $audioPlan
        continue
    }
    ...
    if ($duration -le 0) {
        Write-ShrinkLog -InputPath $file.FullName -Old_MB $oldMB -Status "Skipped-NoDuration" -Detail "ffprobe duration missing/invalid" -OrigVideo $origVideo -OrigAudio $origAudio -OrigDV $origDV -Encode10 ([int]$encode10) -AudioPlan $audioPlan
        continue
    }
    ...
    if ($audCount -le 0) {
        ...
        if ($bestAc3 -gt 0) {
            ...
            $primaryCh = 2
            if ($audStreams -and $primary -ge 1 -and $primary -le $audStreams.Count) {
                $primaryCh = TryParse-Int $audStreams[$primary-1].channels 2
            }
            $ac3Br = if ($primaryCh -ge 6) { $Global:MediaConfig.Ac3Bitrate51K } else { $Global:MediaConfig.Ac3Bitrate20K }
            $rest = @($audioNums | Where-Object { $_ -ne $primary })
            $ordered = @($primary, $primary) + $rest
            $encs = @("ffac3","copy") + (@("copy") * $rest.Count)
            $abs  = @([string]$ac3Br,"0") + (@("0") * $rest.Count)
            $hbAudioArgs = @(
                "--audio", ($ordered -join ","),
                "--aencoder", ($encs -join ","),
                "--ab", ($abs -join ","),
    ...
    if (-not $valid) {
        ...
        if ($origDV -eq 1 -and $newDV -eq 0) {
            $status = "Fail-DV-Lost"
            $failDetail = "DV lost after encode; original kept. " + $failDetail
        }
        Write-ShrinkLog -InputPath $file.FullName -Old_MB $oldMB -New_MB $newMB -Saved_MB $savedMB -Status $status -Strategy $reason -Detail $failDetail -OrigVideo $origVideo -NewVideo $newVideo -OrigAudio $origAudio -NewAudio $newAudio -OrigDV $origDV -NewDV $newDV -Encode10 ([int]$encode10) -AudioPlan $audioPlan
        continue
    }
